...
FROM docker://emscripten/emsdk:3.1.40 AS emsdk-base
ARG EXTRA_CFLAGS
ARG EXTRA_LDFLAGS
ARG FFMPEG_ST
ARG FFMPEG_MT
...
RUN git clone -b $X264_BRANCH https://github.com/ffmpegwasm/x264.git /src
...
ENV X265_BRANCH=3.4
RUN git clone -b $X265_BRANCH https://github.com/ffmpegwasm/x265.git /src
...
RUN git clone -b $LIBVPX_BRANCH https://github.com/ffmpegwasm/libvpx.git /src
...
RUN git clone -b $LAME_BRANCH https://github.com/ffmpegwasm/lame.git /src
...
RUN git clone -b $OGG_BRANCH https://github.com/ffmpegwasm/Ogg.git /src
...
RUN git clone -b $THEORA_BRANCH https://github.com/ffmpegwasm/theora.git /src
...
RUN git clone -b $OPUS_BRANCH https://github.com/ffmpegwasm/opus.git /src
...
RUN git clone -b $VORBIS_BRANCH https://github.com/ffmpegwasm/vorbis.git /src
...
RUN git clone -b $ZLIB_BRANCH https://github.com/ffmpegwasm/zlib.git /src
...
RUN git clone -b $LIBWEBP_BRANCH https://github.com/ffmpegwasm/libwebp.git /src
...
RUN git clone -b $FREETYPE2_BRANCH https://github.com/ffmpegwasm/freetype2.git /src
...
RUN git clone -b $FRIBIDI_BRANCH https://github.com/fribidi/fribidi.git /src
...
RUN git clone -b $HARFBUZZ_BRANCH https://github.com/harfbuzz/harfbuzz.git /src
...
RUN git clone -b $LIBASS_BRANCH https://github.com/libass/libass.git /src
...
RUN git clone -b $FFMPEG_VERSION https://github.com/FFmpeg/FFmpeg.git /src
